...
from hope_payment_gateway.apps.fsp.palpay import (
    AVAILABLE,
    CLOSED,
    DELIVERED,
    IN_TRANSIT,
...
class PalPayClient(FSPClient, metaclass=Singleton):
    ...
    def __init__(self):
        self.set_token()
        self.sender = FinancialServiceProvider.objects.get(vendor_number=config.PALPAY_VENDOR_NUMBER).configuration

    def set_token(self):
        """Set up the token to perform PalPay API calls."""
        url = settings.PALPAY_HOST + "/oauth/accesstoken?grant_type=client_credentials"
        credentials = f"{settings.PALPAY_CLIENT_ID}:{settings.PALPAY_CLIENT_SECRET}"
    ...
    def quote(self, base_payload):
        """Create a quote request to PalPay."""
