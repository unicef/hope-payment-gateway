...
class PalPayClient(FSPClient, metaclass=Singleton):
    ...
    def prepare_transaction(self, base_payload):
        ...
        config = getattr(
            FinancialServiceProvider.objects.get(vendor_number=settings.PALPAY_VENDOR_NUMBER), "configuration", dict
        )
        full_name = " ".join(
            [
                base_payload[value]
        ...
        try:
            payload = {
                ...
                "continueTransIfNotHasWallet": config.get("continue_if_not_has_wallet", True),
                "continueTransIfSkipLimitAmount": config.get("continue_if_skip_limit_amount", True),
                "beneficiary": [
                    {
                        ...
                        "mobile": phone_number,
                        "cityName": base_payload["account-city-name"],
                        "govName": base_payload["account-gov-name"],
