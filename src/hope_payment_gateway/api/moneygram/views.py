...
class MoneyGramWebhook(MoneyGramApi):
    ...
    def post(self, request):
        ...
        try:
            ...
            message = f"Cannot find payment with provided reference {record_key}"
            logger.exception(message)
            return Response(
                {"cannot_find_transaction": message},
                status=HTTP_400_BAD_REQUEST,
            )
        try:
            self.update_record(pr, payload)
        except TransitionNotAllowed:
            logger.exception(f"Transition not allowed for {record_key}")
