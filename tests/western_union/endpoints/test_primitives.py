...
# @_recorder.record(file_path="tests/western_union/endpoints/send_money_store.yaml")
@responses.activate
def test_send_money_store(django_app, admin_user):
    ...
    responses._add_from_file(file_path="tests/western_union/endpoints/send_money_store.yaml")
    payment = PaymentRecord.objects.first()
...
# @_recorder.record(file_path="tests/western_union/endpoints/cancel_request.yaml")
@responses.activate
def test_cancel_request(django_app, admin_user):
    ...
    responses._add_from_file(file_path="tests/western_union/endpoints/cancel_request.yaml")
    payment = PaymentRecord.objects.first()
    payment.transaction_reference_id = "8552593921"
    payment.save()
