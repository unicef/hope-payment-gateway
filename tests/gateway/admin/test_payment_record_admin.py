...
from gateway.admin.utils import assert_has_expected_choices, find_button, get_change_model_url, get_view_handler_url
...
def test_get_queryset(mocker: MockerFixture) -> None:
    ...
    super_get_queryset.assert_has_calls((call(request).select_related("parent__fsp"),))
...
def test_moneygram_choice(admin_client: Client, prl: PaymentRecord) -> None:
    ...
    assert button
    assert_has_expected_choices(button, PaymentRecordAdmin.mg_prepare_payload, PaymentRecordAdmin.mg_create_transaction)
...
@mark.parametrize(
    "url",
    map(
        partial(get_view_handler_url, PaymentRecord),
        (
            PaymentRecordAdmin.wu_prepare_payload,
            PaymentRecordAdmin.wu_send_money_validation,
            PaymentRecordAdmin.wu_send_money,
