...
def user_with_permissions(user):
    permission = Permission.objects.get(codename="can_check_status", content_type__app_label="western_union")
...
def test_wu_status_no_permissions(user, western_union_admin_instance, payment_record):
    factory = RequestFactory()
    request = factory.get("/")
    request.user = user

    with pytest.raises(PermissionDenied):
        western_union_admin_instance.wu_status(western_union_admin_instance, request, payment_record.pk)


@pytest.mark.django_db
@override_config(WESTERN_UNION_VENDOR_NUMBER="12345")
@patch("hope_payment_gateway.apps.gateway.admin.western_union.WesternUnionClient")
def test_wu_status_success(mock_wu_client, user_with_permissions, western_union_admin_instance, payment_record, client):
    mock_response = {
        "title": "mocked title",
    }
    ...
    url = reverse("admin:gateway_paymentrecord_wu_status", args=[payment_record.pk])
    response = client.get(url)

    mock_wu_client.return_value.status.assert_called_once_with(payment_record.fsp_code, False)
...
@pytest.mark.django_db
@override_config(WESTERN_UNION_VENDOR_NUMBER="12345")
def test_wu_status_missing_mtcn(user_with_permissions, western_union_admin_instance, payment_record, client):
