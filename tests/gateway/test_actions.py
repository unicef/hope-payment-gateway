...
@pytest.mark.django_db
def test_yield_rows_formatting():
    from datetime import datetime, date, time
    ...
    record1 = PaymentRecordFactory(record_code=dt)
    record2 = PaymentRecordFactory(record_code=d)
    record3 = PaymentRecordFactory(record_code=t)

    queryset = PaymentRecord.objects.filter(id__in=[record1.id, record2.id, record3.id])
    fields = ["{{ obj.record_code }}"]

    with patch("django.utils.dateformat.format", autospec=True) as mock_format:
        ...
        assert "2023-01-01 12:00:00" in lines[1]
        assert "2023-01-01" in lines[2]
        assert "12:00:00" in lines[3]
        # TODO should be covering the if block for yield_rows but fails to do so
