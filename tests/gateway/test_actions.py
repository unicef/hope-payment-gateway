...
from factories import PaymentRecordFactory, FinancialServiceProviderFactory, UserFactory
...
class TestTemplateExportForm:
    ...
    def test_clean_method(self, input_data, expected_headers, expected_columns):
        form_data = {
            "columns": input_data,
            "header": "",
            "select_across": "0",
...
@pytest.mark.django_db
class TestExportAsTemplateImpl:
    @pytest.fixture
    def sample_queryset(self):
        return PaymentRecord.objects.all()

    def test_basic_export(self, sample_queryset):
    ...
    def test_streaming_export(self, sample_queryset, streaming_enabled):
    ...
    def test_with_custom_options(self, sample_queryset):
    ...
    def test_datetime_formatting(self, field_type, field_value, expected_format):
        record = PaymentRecordFactory(record_code=field_value)
        queryset = PaymentRecord.objects.filter(id=record.id)
        fields = ["{{ obj.record_code }}"]

        response = export_as_template_impl(
            queryset=queryset, fields=fields, options={"datetime_format": expected_format}
        )

        assert isinstance(response, HttpResponse)

    def test_with_custom_dialect(self, sample_queryset):
    ...
    def test_header_generation(self, sample_queryset):
        fields = ["{{ obj.record_code }}", "{{ obj.message }}"]
        header = ["Record Code", "Message"]
    ...
    def test_header_generation_without_header(self, sample_queryset):
        fields = ["{{ obj.record_code }}", "{{ obj.message }}"]
...
class TestExportAsTemplate:
    ...
    @pytest.fixture
    def request_with_data(self, request_with_messages, admin_user):
    ...
    def test_export_with_valid_form(self, modeladmin, request_with_data):
    ...
    def test_export_with_custom_form_class(self, modeladmin, request_with_data):
    ...
    def test_export_with_invalid_form(self, modeladmin, request_with_data):
        post_data = QueryDict(mutable=True)
        post_data.update(
            {
                "action": "export_as_template",
    ...
    def test_export_with_empty_queryset(self, modeladmin, request_with_data):
    ...
    def test_export_with_custom_template(self, modeladmin, request_with_data):
...
@pytest.mark.django_db
class TestMoneyGramUpdateStatus:
    @pytest.fixture
    def modeladmin(self):
        admin = MagicMock(spec=ModelAdmin)
        admin.model = PaymentRecord
    ...
    @pytest.fixture
    def request_with_messages(self, request_factory):
        request = request_factory.post("/")
        middleware = SessionMiddleware(lambda r: None)
        middleware.process_request(request)
        middleware = MessageMiddleware(lambda r: None)
    ...
    def test_moneygram_update_called(self, modeladmin, request_with_messages, moneygram_fsp):
...
class TestMoneyGramRefund:
    ...
    @override_config(MONEYGRAM_VENDOR_NUMBER="MONEYGRAM")
    def test_refund_with_permission(self, modeladmin, request_with_messages, moneygram_fsp):
    ...
    @override_config(MONEYGRAM_VENDOR_NUMBER="MONEYGRAM")
    def test_refund_without_permission(self, modeladmin, request_with_messages, moneygram_fsp):
        records = PaymentRecordFactory.create_batch(2, parent__fsp=moneygram_fsp, fsp_code="TEST123")
        record_ids = [record.id for record in records]
        queryset = PaymentRecord.objects.filter(id__in=record_ids)
    ...
    @override_config(MONEYGRAM_VENDOR_NUMBER="MONEYGRAM")
    def test_refund_with_invalid_form(self, modeladmin, request_with_messages, moneygram_fsp):
        records = PaymentRecordFactory.create_batch(2, parent__fsp=moneygram_fsp, fsp_code="TEST123")
        record_ids = [record.id for record in records]
        queryset = PaymentRecord.objects.filter(id__in=record_ids)
