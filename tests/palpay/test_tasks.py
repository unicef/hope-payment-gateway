...
from hope_payment_gateway.apps.fsp.palpay.tasks import palpay_send_money, palpay_update
...
@patch("hope_payment_gateway.apps.fsp.palpay.tasks.AsyncJob.queue")
def test_send_money_task(mock_class, mg, rec_a, rec_b, total):
    ...
    palpay_send_money(tag="tag", threshold=10)
...
@override_settings(CELERY_TASK_ALWAYS_EAGER=True)
@override_config(PALPAY_VENDOR_NUMBER="67890")
@patch("hope_payment_gateway.apps.fsp.palpay.tasks.PalPayClient.query_status")
def test_send_palpay_update(mock_class, mg, rec_a, rec_b, total):
