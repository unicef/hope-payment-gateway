...
def user_with_permissions(user):
    permission = Permission.objects.get(codename="can_search_request", content_type__app_label="western_union")
...
def test_wu_search_request_no_permissions(user, western_union_admin_instance, payment_record):
    factory = RequestFactory()
    request = factory.get("/")
    request.user = user

    with pytest.raises(PermissionDenied):
        western_union_admin_instance.wu_search_request(western_union_admin_instance, request, payment_record.pk)


@pytest.mark.django_db
@override_config(WESTERN_UNION_VENDOR_NUMBER="12345")
@patch("hope_payment_gateway.apps.gateway.admin.western_union.WesternUnionClient")
def test_wu_search_request_success(
    ...
    url = reverse("admin:gateway_paymentrecord_wu_search_request", args=[payment_record.pk])
    ...
    mock_wu_client.return_value.search_request.assert_called_once_with("TEST_SYSTEM", "1234567890")


@pytest.mark.django_db
@override_config(WESTERN_UNION_VENDOR_NUMBER="12345")
def test_wu_search_request_missing_mtcn(user_with_permissions, western_union_admin_instance, payment_record, client):
